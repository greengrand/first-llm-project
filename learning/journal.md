...
### Итог от ассистента — 2025-12-04

Сегодня выполнено практическое исследование поведения LLM при разных значениях параметра temperature и отладка стабильности запросов:
- Добавлен и отлажен скрипт examples/temperature_test.py с retry/backoff для обработки 429 (rate limit).
- Проведены прогонные тесты: temperature 0.0/0.2 дают стабильные ответы, 0.7 — более вариативный текст; при 1.0 обнаружен 429 (лимит RPM).
- Проверен json_prompt_test.py — модель вернула валидный JSON, парсинг прошёл успешно.
- Результаты сохранены: лог в logs/; пример результата — results/temperature_test_results.json.
- Выводы: для строгого формата лучше держать temperature 0.0–0.2; для креатива — 0.7+, но нужно контролировать частоту запросов и иметь retry/backoff. План: Day 4 — реализовать JSON-валидатор и интегрировать проверку в endpoint /summarize.

Комментарий: всё выполнено аккуратно — баг рекурсии исправлен, добавлена защита от кратковременных пиков запросов. Рекомендую в следующих прогонах ставить небольшую паузу между итерациями (например sleep 1s) и не забыть ротировать/ревокировать ключи при подозрениях их компрометации.
